...
const categoryList = []
class WortenCategory implements AcqCategory {
    ...
    async getCategory(filter: object): Promise<Array<Category>> {
        ...
        const url: string = 'https://www.worten.pt';
        try {
            this.leafTraverse = new LeafTraverse().make('Worten')    // siteName
            // '' 안에 사이트 명을 작성
            /*
            ...
            detailPage('div.slider.category-links__slider.wrapper > div.swiper-container.grid.swiper-container-initialized.swiper-container-horizontal.swiper-container-free-mode > div.swiper-wrapper > div').each((index, content) => {      //cateogry element
                let parentDiv = detailPage(content)

                let cateUrl : string = parentDiv.find('> article > a ').attr('href'); // category Url 엘리먼트 받아오기
                // ex > let cateUrl = detailPage('categoryUrlElement').text();
                let cateName : string =parentDiv.find('> article > a > p').text(); // category name 엘리먼트 받아오기
